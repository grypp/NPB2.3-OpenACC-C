...
int main(int argc, char **argv) {
	...
	printf("NA: %ld, NZ: %ld\n", NA, NZ);
#ifdef ULCC_ENABLE
	printf("With ULCC support\n");
#else
	printf("Without ULCC support\n");
#endif

#ifdef ULCC_ENABLE
	if(cc_init() < 0)
	{
		printf("ulcc init failed!\n");
		return -1;
	}
	cc_cacheregn_t regn;
	unsigned long st, ed;

	/* weak locality - a, colidx */
	cc_cacheregn_clr(&regn);
	cc_cacheregn_set(&regn, 0, 17, 1);
	st = (unsigned long)(&a[0]);
	ed = (unsigned long)(&a[NZ+1]);
	if(cc_remap(&st, &ed, 1, &regn, 0, NULL) < 0)
	{
		printf("Failed to map a[]!\n");
		cc_fini();
		return -1;
	}
	st = (unsigned long)(&colidx[0]);
	ed = (unsigned long)(&colidx[NZ+1]);
	if(cc_remap(&st, &ed, 1, &regn, 0, NULL) < 0)
	{
		printf("Failed to map colidx[]!\n");
		cc_fini();
		return -1;
	}
	...
	/* other */
	cc_cacheregn_clr(&regn);
	cc_cacheregn_set(&regn, 19, 62, 1);
	unsigned long *pst = malloc(sizeof(unsigned long) * 12);
	unsigned long *ped = malloc(sizeof(unsigned long) * 12);
	pst[0] = (unsigned long)(&p[0]);
	ped[0] = (unsigned long)(&p[NA+2+1]);
	pst[1] = (unsigned long)(&rowstr[0]);
	ped[1] = (unsigned long)(&rowstr[NA+1+1]);
	pst[2] = (unsigned long)(&iv[0]);
	ped[2] = (unsigned long)(&iv[2*NA+1+1]);
	pst[3] = (unsigned long)(&arow[0]);
	ped[3] = (unsigned long)(&arow[NZ+1]);
	pst[4] = (unsigned long)(&acol[0]);
	ped[4] = (unsigned long)(&acol[NZ+1]);
	pst[5] = (unsigned long)(&v[0]);
	ped[5] = (unsigned long)(&v[NA+1+1]);
	pst[6] = (unsigned long)(&aelt[0]);
	ped[6] = (unsigned long)(&aelt[NZ+1]);
	pst[7] = (unsigned long)(&x[0]);
	ped[7] = (unsigned long)(&x[NA+2+1]);
	pst[8] = (unsigned long)(&z[0]);
	ped[8] = (unsigned long)(&z[NA+2+1]);
	pst[9] = (unsigned long)(&q[0]);
	ped[9] = (unsigned long)(&q[NA+2+1]);
	pst[10] = (unsigned long)(&r[0]);
	ped[10] = (unsigned long)(&r[NA+2+1]);
	pst[11] = (unsigned long)(&w[0]);
	ped[11] = (unsigned long)(&w[NA+2+1]);
	if(cc_remap(pst, ped, 12, &regn, 0, NULL) < 0)
	{
		printf("Failed to map the rest arrays!\n");
		cc_fini();
		return -1;
	}
	/* kaibo_add_end */
